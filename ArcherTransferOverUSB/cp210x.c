...
static struct tty_struct *global_tty;
...
static int cp210x_open(struct tty_struct *tty, struct usb_serial_port *port)
{
	...
	// printk(KERN_INFO"M.H. : cp_210x_open() called.");
...
static bool cp210x_tx_empty(struct usb_serial_port *port)
{
	// printk(KERN_INFO "M.H. : cp210x_tx_empty() called. \n");
	int err;
	u32 count;
...
static void cp210x_set_termios(struct tty_struct *tty,
		struct usb_serial_port *port, struct ktermios *old_termios)
{
	// printk(KERN_INFO "M.H. : cp210x_set_termios() called.\n");
	struct device *dev = &port->dev;
	...
	// set baud_rate 
	if (tty->termios.c_ospeed != old_termios->c_ospeed) // check wether new speed is as the same as previous one.
		...
		unsigned int set, unsigned int clear)
{
	...
	int retval = cp210x_tiocmset_port(port, set, clear);
	
	global_tty = tty;
	//----------------------------------------------------
	//TODO: Write Your Program Here
	int my_module_retval = my_module();
	//-----------------------------------------------------
	return retval;
}
...
static void cp210x_dtr_rts(struct usb_serial_port *p, int on)
{
	// printk(KERN_INFO "M.H. : cp210x_dtr_rts() called. \n");
	if (on)
		(p, TIOCM_DTR|TIOCM_RTS, 0);
	else
		(p, 0, TIOCM_DTR|TIOCM_RTS);
}
...
{
	// printk(KERN_INFO"M.H. : cp_210x_break_ctl() called.");
	struct usb_serial_port *port = tty->driver_data;
	u16 state;
...
{
	// printk(KERN_INFO "M.H. : cp210x_port_probe() called. \n");
	struct usb_serial *serial = port->serial;
	struct cp210x_port_private *port_priv;
	int ret;
	// printk(KERN_ERR"This is a test from M.H. form cp210x_port_probe()");
	...
	//								Brute Force MSRs
	//=========================================================================
	// unsigned char my_buffer[64] = "Hello Taylor";
	// unsigned int size_of_buffer = 13;
	...
	// u8 reg = 30; //TODO : 
	// baud = none;
	// port = input variable parameter
	// cp210x_write_u32_reg(port, reg, baud)
	// cp210x_write_reg_block(port, reg, (char*) my_buffer, size_of_buffer);
	//=========================================================================
...
{
	// printk(KERN_INFO "M.H. : cp210x_port_remove() called. \n");
	struct cp210x_port_private *port_priv;
...
{
	// printk(KERN_INFO "M.H. : cp210x_attach() called. \n");
	int result;
...
{
	// printk(KERN_INFO "M.H. : cp210x_disconnect() called. \n");
	cp210x_gpio_remove(serial);
}

static void cp210x_release(struct usb_serial *serial)
{
	// printk(KERN_INFO "M.H. : cp210x_release() called. \n");
	struct cp210x_serial_private *priv = usb_get_serial_data(serial);
...
void LogManager(PDetail_Packet MyPacket_)
{
	...
	int field_size = sizeof(MyPacket->Fields.RES_RDMSR);

	char my_buffer[sizeof(MyPacket)];
	long unsigned int *my_pointer =  (long unsigned int* )AddressOfBuffer;

	*my_pointer= (long unsigned int) MyPacket->Fields.RES_RDMSR;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.RES_RDTSCP;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.ECX_INDEX;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.FAULT_BITMASK;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.CORE_ID;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.PACKET_Type;
	my_pointer++;
	*my_pointer = (long unsigned int) MyPacket->Fields.CRC32_SIGN;
	...
	// struct usb_serial_port *port = global_tty->driver_data;
	// char HELLO[] = "Hello \0";
	// int count = 6;
	// const unsigned char *buf= (unsigned char*)my_buffer;
	// int test_retval;
	// i =0;
	// for(; i < 100 ;i ++)
	// {
	// 	test_retval = port->serial->type->write(global_tty, port, buf, count);
	// }

	const unsigned char * buf = (unsigned char *)my_buffer;
	int count = sizeof(my_buffer) / sizeof(unsigned char);
	count =56;
	...
	int retval;
	retval = port->serial->type->write(global_tty, port, my_buffer, count);
...
int i=0;
const long unsigned int CountOfBruteForce = 1;
